...
class AssetSerializer(serializers.HyperlinkedModelSerializer):
    ...
    def get_last_submission(self, obj):
        count = obj.assetassetsubscription_set.count()  # or use obj.deployment__submission_count if available
        if not count:
            return None

        submissions = DataViewSet().get_submissions(
        ...
        return results[0].get("end") if results else None
